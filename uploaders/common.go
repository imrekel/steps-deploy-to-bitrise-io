...
func uploadArtifact(uploadURL, artifactPth, contentType string) error {
	...
	return retry.Times(3).Wait(5).Try(func(attempt uint) error {
		...
		// Do not add the Content-Type header for Google Cloud Storage uploads (the signature of the presigned URLs
		// generated by the website does not contain the content-type so setting them here would make the upload request fail)
		if contentType != "" && !strings.HasPrefix(uploadURL, "https://storage.googleapis.com") {
